...
static A_0: &'static [Among<Context>; 161] = &[
    ...
    Among("?", -1, 1, None),
    Among("\\", -1, 1, None),
    Among("\u{060C}", -1, 1, None),
    Among("\u{061B}", -1, 1, None),
    Among("\u{061F}", -1, 1, None),
    ...
    Among("\u{066A}", -1, 1, None),
    Among("\u{066B}", -1, 1, None),
    Among("\u{066C}", -1, 1, None),
...
fn r_Normalize_pre(env: &mut SnowballEnv, context: &mut Context) -> bool {
    let mut among_var;
    for _ in 0..(env.current.chars().count() as i32) {
        'lab0: loop {
            let v_2 = env.cursor;
            'lab1: loop {
                env.bra = env.cursor;
...
fn r_Checks1(env: &mut SnowballEnv, context: &mut Context) -> bool {
    let mut among_var;
    context.i_word_len = (env.current.chars().count() as i32);
    env.bra = env.cursor;
