...
fn r_mark_regions(env: &mut SnowballEnv, context: &mut Context) -> bool {
    ...
    'lab0: loop {
        'lab1: loop {
            ...
            env.cursor = v_2;
            'lab3: loop {
                if (env.cursor + 2 >= env.limit || env.current.as_bytes()[(env.cursor + 2) as usize] as u8 >> 5 != 3 as u8 || ((331776 as i32 >> (env.current.as_bytes()[(env.cursor + 2) as usize] as u8 & 0x1f)) & 1) == 0) {
                    break 'lab3;
                }
...
fn r_postlude(env: &mut SnowballEnv, context: &mut Context) -> bool {
    ...
    'replab0: loop{
        ...
        'lab1: for _ in 0..1 {
            env.bra = env.cursor;
            if (env.cursor >= env.limit || env.current.as_bytes()[(env.cursor + 0) as usize] as u8 >> 5 != 2 as u8 || ((35652096 as i32 >> (env.current.as_bytes()[(env.cursor + 0) as usize] as u8 & 0x1f)) & 1) == 0) {among_var = 4;}
            else {
